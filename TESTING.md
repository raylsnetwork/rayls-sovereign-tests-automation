...
- The sibling repos cloned into the **same parent directory** as this repo:
  - `rayls-sovereign-relayer` (brings up the local stack)
  - `rayls-sovereign-contracts` (deploys the contracts; provides the local `.env`)
  - `rayls-sovereign-gnark-api` (Groth16 verifier artifacts)
...
```bash
cd ../rayls-sovereign-tests-automation
...
```bash
cp ../rayls-sovereign-contracts/.env ./.env
```
...
- **Tests can't reach the chain / `.env` errors** — make sure `.env` exists and its RPC URLs match
  the running stack (re-copy from `../rayls-sovereign-contracts/.env`).
