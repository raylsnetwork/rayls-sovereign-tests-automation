...
#
# Copies Solidity verifier contracts from rayls-sovereign-gnark-api/last_build/
# to rayls-sovereign-contracts/src/rayls-protocol/Enygma/ directories.
#
...
GNARK_DIR="${GNARK_DIR:-$BASE_DIR/rayls-sovereign-gnark-api}"
CONTRACTS_DIR="${CONTRACTS_DIR:-$BASE_DIR/rayls-sovereign-contracts}"
...
if [ ! -d "$GNARK_BUILD" ]; then
    ...
    echo "  Make sure rayls-sovereign-gnark-api is cloned alongside this repo." >&2
    exit 1
fi

if [ ! -d "$CONTRACTS_DIR" ]; then
    echo "Error: contracts directory not found at $CONTRACTS_DIR" >&2
    echo "  Make sure rayls-sovereign-contracts is cloned alongside this repo." >&2
    exit 1
fi
...
if [ $mismatched -gt 0 ] && [ "$DRY_RUN" = false ]; then
    ...
    echo "  cd $BASE_DIR/rayls-sovereign-relayer && docker compose -f docker-compose.dev-local.yml build contracts"
fi
