...
task('deployPlaygroundErc721', 'Deploys Playground Erc721 on the PN')
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    LOGGER.log(`Token Deployed At Address ${await tokenPN.getAddress()}`);
    LOGGER.log('Token Deployer Address: ', wallet.address);

    let tx = await tokenPN.mint(wallet.address, 0, { gasLimit: 5000000 });
    await tx.wait();
    LOGGER.log(`Minted token with id 0 to ${wallet.address}`);
    ...
    await (await registrar.registerToken(tokenAddress, 2, false, { gasLimit: 5000000 })).wait();
    ...
    LOGGER.log("To use the token on the public chain, please submit it for approval by the PN operator with the command:");
    LOGGER.log(`\$ npx hardhat addToken --pn ${taskArgs.pn} --token-address ${await tokenPN.getAddress()}`);
    LOGGER.log("Then approve it using:");
    LOGGER.log(`\$ npx hardhat approveRaylsNodeToken --pn ${taskArgs.pn} --private-address ${await tokenPN.getAddress()} --status 1`);
